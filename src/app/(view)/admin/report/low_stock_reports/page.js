...
const ReportsLowStockProduct = () => {
    ...
    const Category = module_settings.filter(moduleI => moduleI.table_name === 'purchase')
    const columnListSelected = Category[0]?.quantity

    console.log(columnListSelected, 'columnListSelected')
    console.log(typeof(columnListSelected), 'columnListSelected')

    const { data: purchase_product_stock_list = [], isLoading, refetch } = useQuery({
        queryKey: ['purchase_product_stock_list'],
        queryFn: async () => {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/purchase/purchase_product_stock_list`
            );
            const data = await res.json();
    
            // Filter data to include only items with available_quantity < 5
            return data.filter(item => item.available_quantity < columnListSelected);
        }
    });
        ...
        axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/purchase/purchase_product_stock_list_search`, {
            ...
            .then(response => {
                ...
                const filteredResults = response.data.results.filter(
                    item => parseFloat(item.available_quantity) < columnListSelected
                );
    ...
    const stock_pdf_download = async () => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/stock/stock_pdf`, {
                method: 'POST',
                headers: {
    ...
    const stock_print = async () => {
        try {
            ...
            const html = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/stock/stock_print`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
export default ReportsLowStockProduct;