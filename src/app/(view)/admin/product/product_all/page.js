...
const ListProduct = ({ searchParams }) => {
    ...
    console.log(moduleInfo.filter(moduleI => moduleI.controller_name === 'product'))
    const brandList = moduleInfo.filter(moduleI => moduleI.controller_name === 'product')
    ...
    const Category = module_settings.filter(moduleI => moduleI.table_name === 'product')
    ...
    const columnNames = products && products.length > 0 ? Object.keys(products[0]) : [];
    ...
    const product_column_change = (selectedItems) => {
    ...
    //     const bodyData = await Promise.all(searchResults.map(async (category, i) => {
    ...
    const product_excel_download = async () => {
        ...
        if (proceed) {
            axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/product/product_delete/${id}`)
                .then(response => {
                    ...
                    // Update the searchResults array by removing the deleted product
    ...
    const product_PDF_download = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Error generating PDF In product');
            }
                        ...
                        <div class="body-content bg-light">
                            ...
                            <div class=" border-primary shadow-sm border-0">
                                <div class=" card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                                    <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Product Search</h5>
                                    ...
                                    <form class="">
                                        <div class="col-md-10 offset-md-1">
                                            ...
                                            <div className='form-group row student'>
                                                <label class="col-form-label col-md-3"><strong>Product Name:</strong></label>
                                                <div class="col-md-3">
                                                    <input
                                                        value={searchQuery}
                                                        onChange={(e) => setSearchQuery(e.target.value)}
                                                        type="text" name="student_id" class="form-control form-control-sm  alpha_space student_id" id="student_id" placeholder="Product Name" />
                                                </div>
                                            ...
                                            <div class="form-group row student">
                                                ...
                                                <div className="col-md-9">
                                                    ...
                                                    <Select
                                                        ...
                                                        onChange={product_column_change}
                                                    />
...
<input type="text" name="extra_column" class="form-control form-control-sm  alpha_space extra_column" id="extra_column" placeholder="Extra Column" />
                                        ...
                                        <div class="form-group row">
                                            <div class="offset-md-2 col-md-10 float-left">
                                                <input type="button" onClick={product_search} name="search" class="btn btn-sm btn-info search_btn mr-2" value="Search" />
                                                <input type="button" onClick={product_print} name="search" class="btn btn-sm btn-success print_btn mr-2" value="Print" />
                                                <button class="btn btn-sm btn-indigo pdf_btn mr-2" style={buttonStyles} onClick={product_PDF_download} disabled={loading}>
                                                    {loading ? 'Generating PDF...' : 'Generate PDF'}
                                                </button>
                                                {errorr && <p>Error: {errorr}</p>}
                                                <input
                                                    type="button"
                                                    onClick={product_excel_download}
                                                    name="search"
                        ...
                        <div class=" border-primary  shadow-sm border-0">
                            <div class="card-header   custom-card-header py-1  clearfix bg-gradient-primary text-white">
                                ...
                                <div class="card-title card-header-color font-weight-bold mb-0  float-right">
                                    <Link href={`/Admin/product/product_create?page_group=${page_group}`} className="btn btn-sm btn-info">Back to Product Create</Link>
                                </div>
                            </div>
                                                                    ...
                                                                    column === 'status_id' ? (
                                                                        ...
                                                                        : column === 'action' ? (
                                                                            ...
                                                                            <div className="flex items-center ">
                                                                                <Link href={`/Admin/product/product_edit/${brand.id}?page_group=${page_group}`}>
                                                                                ...
                                                                                <Link href={`/Admin/product/product_copy/${brand.id}?page_group=${page_group}`}>
                                                                                ...
                                                                                {filteredBtnIconDelete.map((filteredBtnIconDelete => (
                                                                                    <button
                                                                                        ...
                                                                                        title='Delete'
                                                                                        onClick={() => product_delete_searching(brand.id)}
                                                                                        style={{ width: "35px ", height: '30px', marginLeft: '5px', marginTop: '5px' }}
                                    ...
                                    <>
                                        <div className=" d-flex justify-content-between mb-2">
                                            ...
                                            <div class="pagination float-right pagination-sm border">
                                                {
                                                    currentPage - 3 >= 1 && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/product/product_all?page=${1}`}>‹ First</Link>
                                                    )
                                                }
                                                {
                                                    currentPage > 1 && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/product/product_all?page=${activePage - 1}`}>&lt;</Link>
                                                    )
                                                }
                                                {
                                                    pageNumber.map((page) =>
                                                        <Link
                                                            key={page}
                                                            href={`/Admin/product/product_all?page=${page}`}
                                                ...
                                                }
                                                {
                                                    currentPage + 3 <= totalPages && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/product/product_all?page=${totalPages}`}>Last ›</Link>
                                                    )
                                                }
                                            </div>
                                                                            ...
                                                                            column === 'file_path' ? (
                                                                                ...
                                                                                <>
                                                                                    <img
                                                                                        className=" img-thumbnail"
                                                                                        style={{ width: '100px' }}
                                                                                        src={`${process.env.NEXT_PUBLIC_API_URL}:5003${product.file_path}`}
                                                                                        alt="No Image"
                                                                                    />
                                                                                </>
                                                                            ) :
                                                                                column === 'status_id' ? (
                                                                                    product[column] === 1 ? 'Active' : product[column] === 2 ? 'Inactive' : product[column] === 3 ? 'Pending' : 'Unknown'
                                                                                )
                                                                                    :
                                                                                    column === 'action' ? (
                                                                                        <div className="flex items-center ">
                                                                                            <Link href={`/Admin/product/product_edit/${product.id}?page_group=${page_group}`}>
                                                                                                {filteredBtnIconEdit?.map((filteredBtnIconEdit => (
                                                                                                    <button
                                                                                                        key={filteredBtnIconEdit.id}
                                                                                                        title='Edit'
                                                                                            ...
                                                                                            <Link href={`/Admin/product/product_copy/${product.id}?page_group=${page_group}`}>
                                                                                                {filteredBtnIconCopy.map((filteredBtnIconEdit => (
                                                                                                    <button
                                                                                                        key={filteredBtnIconEdit.id}
                                                                                                        title='Copy'
