...
// const EmployeLoanPaymentUpdate = ({ id }) => {
...
//         loan: '', account: '', reference: '', amount: '', interest: '', aviable_balance: '', due: '', payable_amount: '', payment_date: '', note: '', status: '', modified_by: created, img: ''
//     });
...
//         queryFn: async () => {
//             const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/employe_loan_payment/employe_loan_payment_all/${id}`);
...
//             loan: loanPayment[0]?.employe_loan_id || '',
//             account: loanPayment[0]?.account || '',
//             reference: loanPayment[0]?.reference || '',
//             amount: loanPayment[0]?.amount || '',
//             interest: loanPayment[0]?.interest || '',
//             aviable_balance: loanPayment[0]?.aviable_balance || '',
//             due: loanPayment[0]?.due || '',
//             payable_amount: loanPayment[0]?.payable_amount || '',
//             payment_date: loanPayment[0]?.payment_date || '',
//             note: loanPayment[0]?.note || '',
//             status: loanPayment[0]?.status || '',
//             img: loanPayment[0]?.img || '',
...
//         fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/employe_loan_payment/employe_loan_payment_update/${id}`, {
//             method: 'POST',
//             headers: {
...
//                                 <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Update Employe Loan Payment</h5>
//                                 <div class="card-title font-weight-bold mb-0 card-header-color float-right">
//                                     <Link href={`/Admin/employe_loan_payment/employe_loan_payment_all?page_group=${page_group}`} class="btn btn-sm btn-info">Back Employe Loan Payment List</Link></div>
//                             </div>
...
//                                                     />
//                                                     {
//                                                         reference && <p className='text-danger'>{loan}</p>
//                                                     }
...
const EmployeLoanPaymentUpdate = ({ id }) => {
    ...
    const [assetInfo, setAssetInfo] = useState({
        loan: '', account: '', reference: '', amount: '', interest: '', aviable_balance: '', due: '', payable_amount: '', payment_date: '', note: '', status: '', modified_by: created, img: '', employee_id: ''
    });
    ...
    const totalPayableForAccount6 = loanAll
        .filter(item => item.account == assetInfo.account && item.employee_id == assetInfo.employee_id)  // Filter the data for account assetInfo.account
    ...
    const totalPayableForLoanPaymentList = loanPaymentList
        .filter(item => item.account == assetInfo.account && item.employe_id == assetInfo.employee_id)  // Filter the data for account assetInfo.account
        .reduce((sum, curr) => sum + curr.payable_amount, 0);  // Sum up payable_amount
    ...
    useEffect(() => {
        setAssetInfo({
            ...
            status: loanPayment[0]?.status || '',
            img: loanPayment[0]?.img || '',
            employee_id: loanPayment[0]?.employe_id || '',
    ...
    useEffect(() => {
        if (assetInfo.account) {
            ...
            if (item) {
                const updatedItem = {
                    ...item,
                    amount: ((item.amount || 0) + parseFloat(loanPayment[0]?.payable_amount)) + parseFloat(assetInfo.payable_amount) // Add 100 to amount, 
                };
    ...
    }, [account_head, assetInfo.payable_amount, assetInfo.account, loanPayment]); // Trigger when selectedEntryType changes
