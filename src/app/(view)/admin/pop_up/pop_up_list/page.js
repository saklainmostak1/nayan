...
const PopUpList = ({ searchParams }) => {
    ...
    const { data: front_service_box = [], refetch, isLoading
        ...
        queryFn: async () => {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/pop_up/pop_up_list`)
    ...
    console.log(moduleInfo.filter(moduleI => moduleI.controller_name === 'pop_up'))
    const brandList = moduleInfo.filter(moduleI => moduleI.controller_name === 'pop_up')
    ...
    const caregory_list = async () => {
        const url = `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/front_service_box/pop_up_list_paigination/${currentPage}/${dataPerPage}`;
    ...
    const front_service_box_delete = id => {
        ...
        if (proceed) {
            fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/pop_up/pop_up_delete/${id}`, {
                method: "POST",
    ...
    const pop_up_search = () => {
        ...
        setLoading(true);
        axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/pop_up/pop_up_search`, {
    ...
    const front_service_box_pdf_download = async () => {
        ...
        const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/pop_up/pop_up_search`, {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/pop_up/pop_up_list_pdf`, {
                method: 'POST',
                headers: {
    ...
    const front_service_box_print = async () => {
        try {
            ...
            const html = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/pop_up/pop_up_list_print`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const front_service_box_excel_download = async () => {
        try {
            ...
            const columns = [
                ...
                { header: 'Pop Up Name', key: 'popUpName', width: 30 },
                { header: 'Pop Up Delay', key: 'popUpDelay', width: 10 },
                { header: 'Pop Up Start-End', key: 'startEnd', width: 20 },
                { header: 'Status', key: 'status', width: 20 },
                { header: 'Photo', key: 'photo', width: 15 },
                { header: 'Created By/Date', key: 'createdDate', width: 20 },
            ];
            ...
            for (const [index, category] of searchResults.entries()) {
                const row = {
                    slNo: index + 1,
                    popUpName: category.title_en || '',
                    popUpDelay: category.pop_up_delay || '',
                    startEnd: `${category.pop_up_start_date.slice(0, 10) || ''} - ${category.pop_up_end_date.slice(0, 10) || ''}`,
                ...
                if (category.img) {
                    try {
                        ...
                        worksheet.addImage(imageId, {
                            tl: { col: 5, row: addedRow.number - 1 }, // Adjust the position
                            ext: { width: 40, height: 15 }, // Adjust size as needed
                        });
                    } catch (imageError) {
            ...
            link.href = url;
            link.download = `front_service_box_${Date.now()}.xlsx`; // Unique file name
    ...
    const front_service_box_word_download = async () => {
        try {
            ...
            const dataRows = await Promise.all(searchResults.map(async (category, index) => {
                const pop_up_delay = category.pop_up_delay != null ? `${category.pop_up_delay}` : ''; // 
                ...
                return new TableRow({
                    children: [
                        ...
                        new TableCell({
                            children: [new Paragraph({ text: category.title_en ? `${category.title_en}` : '' })], // Name
                            borders: {},
                        }),
                        new TableCell({
                            children: [new Paragraph({ text: pop_up_delay ? pop_up_delay : '' })], // Email
                            borders: {},
                        }),


                        new TableCell({
                            children: [new Paragraph({ text: `${category.pop_up_start_date.slice(0, 10)} - ${category.pop_up_end_date.slice(0, 10)}` })], // Contact Number
                            borders: {},
                        }),
                        new TableCell({
        ...
        <div class="col-md-12 bg-light p-4">
                ...
                <div className="body-content bg-light">
                    <div className="border-primary shadow-sm border-0">
                        <div className="card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                            <h5 className="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Pop Up Search</h5>
                        </div>
                            ...
                            <form >
                                <div className="col-md-10 offset-md-1">
                                    ...
                                    <div className="form-group row">
                                        <label className="col-form-label col-md-2"><strong>Pop Up Name:</strong></label>
                                        <div className="col-md-4">
                                        ...
                                        <label className="col-form-label col-md-2"><strong>Pop Up Delay:</strong></label>
                                        <div className="col-md-4">
            ...
            <div className='card'>
                <div className="body-content bg-light">
                    <div className="border-primary shadow-sm border-0">
                        <div className="card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                            <h5 className="card-title font-weight-bold mb-0 card-header-color float-left mt-1">List Pop Up</h5>
                            <div className="card-title font-weight-bold mb-0 card-header-color float-right">
                                <Link href={`/Admin/pop_up/pop_up_create?page_group=dynamic_website`} className="btn btn-sm btn-info">Pop Up Create</Link>
                            </div>
                        </div>
                                                ...
                                                <table className="table  table-bordered table-hover table-striped table-sm">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th>Pop Up Name</th>
                                                            <th>Pop Up Delay</th>
                                                            <th>Pop Up <br />Start-End</th>
                                                            <th>Status</th>
                                                            <th>Created <br /> By/Date</th>
                                                            <th>Action</th>
                                                        </tr>
                                                    </thead>
                                                    ...
                                                    <tbody>
                                                        {isLoading ? <div className='text-center'>
                                                            ...
                                                            searchResults.map((front, i) => (
                                                                <tr key={front.id}>
                                                                    <td>{i + 1}</td>
                                                                    <td>{front.title_en}</td>
                                                                    <td>{front.pop_up_delay}</td>
                                                                    <td>{front.pop_up_start_date.slice(0, 10)} <br /> {front.pop_up_end_date.slice(0, 10)}</td>
                                                                    <td>{front.status_text}</td>
                                                                    <td>{front.created_by} <br />{front.created_date.slice(0, 10)} </td>
                                                                    <td>
                                                                        <div className="flex items-center ">
                                                                            <Link href={`/Admin/pop_up/pop_up_edit/${front.id}?page_group=${page_group}`}>
                                    ...
                                    <div className='table-responsive'>
                                        <div className=" d-flex justify-content-between">
                                            ...
                                            <div class="pagination float-right pagination-sm border">
                                                {
                                                    currentPage - 3 >= 1 && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/pop_up/pop_up_all?page=${1}`}>‹ First</Link>
                                                    )
                                                }
                                                {
                                                    currentPage > 1 && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/pop_up/pop_up_all?page=${activePage - 1}`}>&lt;</Link>
                                                    )
                                                }
                                                {
                                                    pageNumber.map((page) =>
                                                        <Link
                                                            key={page}
                                                            href={`/Admin/pop_up/pop_up_all?page=${page}`}
                                                ...
                                                }
                                                {
                                                    currentPage + 3 <= totalPages && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/pop_up/pop_up_all?page=${totalPages}`}>Last ›</Link>
                                                    )
                                                }
                                            </div>
