...
const PeriodList = ({ searchParams }) => {
    ...
    console.log(moduleInfo.filter(moduleI => moduleI.controller_name === 'period'))
    const brandList = moduleInfo.filter(moduleI => moduleI.controller_name === 'period')
    ...
    const Category = module_settings.filter(moduleI => moduleI.table_name === 'period')
    ...
    const columnNames = periods && periods.length > 0 ? Object.keys(periods[0]) : [];
    ...
    const period_excel_download = async () => {
        ...
        if (proceed) {
            axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/period/period_delete/${id}`)
                .then(response => {
                    ...
                    // Update the searchResults array by removing the deleted period
                    setSearchResults(prevResults => prevResults.filter(period => period.id !== id));
                })
                .catch(error => {
                        ...
                        <div class="body-content bg-light">
                            ...
                            <div class=" border-primary shadow-sm border-0">
                                <div class=" card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                                    <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Period Search</h5>
                                    <div class="card-title font-weight-bold mb-0 card-header-color float-right">

                                        <Link href={`/Admin/period/period_create?page_group=${page_group}`} class="btn btn-sm btn-info">Back To Create Period</Link>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <form class="">
                                        <div class="col-md-10 offset-md-1">
                                            ...
                                            <div className='form-group row student'>
                                                <label class="col-form-label col-md-3"><strong>Period Name:</strong></label>
                                                <div class="col-md-3">
                                                    <input
                                                        value={searchQuery}
                                                        onChange={(e) => setSearchQuery(e.target.value)}
                                                        type="text" name="student_id" class="form-control form-control-sm  alpha_space student_id" id="student_id" placeholder="Period Name" />
                                                </div>
...
<input type="text" name="extra_column" class="form-control form-control-sm  alpha_space extra_column" id="extra_column" placeholder="Extra Column" />
                                        ...
                                        <div class="form-group row">
                                            <div class="offset-md-2 col-md-10 float-left">
                                                <input type="button" onClick={period_search} name="search" class="btn btn-sm btn-info search_btn mr-2" value="Search" />
                                                <input type="button" onClick={period_print} name="search" class="btn btn-sm btn-success print_btn mr-2" value="Print" />
                                                <button class="btn btn-sm btn-indigo pdf_btn mr-2" style={buttonStyles} onClick={period_PDF_download} disabled={loading}>
                    ...
                    <div className='card mb-4'>
                        <div class=" border-primary  shadow-sm border-0">
                            <div class="card-header   custom-card-header py-1  clearfix bg-gradient-primary text-white">
                                <h5 class="card-title card-header-Period font-weight-bold mb-0  float-left mt-1">Period List</h5>
                                <div class="card-title card-header-Period font-weight-bold mb-0  float-right"> <Link href="/Admin/period/period_create" class="btn btn-sm btn-info">Create Period</Link> </div>
                            </div>
...
// import DatePicker from 'react-datepicker';
// import 'react-datepicker/dist/react-datepicker.css';
// // import '../../../admin_layout/modal/fonts/Nikosh.ttf'
...
//                     <button onClick={period_PDF_download} disabled={loading}>
//                         {loading ? 'Generating PDF...' : 'Generate PDF'}
//                     </button>
//                     {errorr && <p>Error: {errorr}</p>}
//                 </div>
...
//                                     </div>
//                                     <label class="col-form-label col-md-2"><strong>Period Name:</strong></label>
//                                     <div class="col-md-4">
//                                         <input
//                                             value={searchQuery}
//                                             onChange={(e) => setSearchQuery(e.target.value)}
...
//                                     <input type="button" name="search" onClick={period_PDF_download} style={buttonStyles} class="btn btn-sm btn-indigo pdf_btn mr-2" value="Download PDF" />
...
//                     <h5 class="card-title card-header-Period font-weight-bold mb-0  float-left mt-1">Period List</h5>
//                     <div class="card-title card-header-Period font-weight-bold mb-0  float-right"> <a href="" class="btn btn-sm btn-info">Create Period</a> </div>
//                 </div>
