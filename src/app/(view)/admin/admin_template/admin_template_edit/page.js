...
const AdminTemplateEdit = ({ id }) => {
	...
	//     }
	//   };
	//   const admin_template_image_remove = (index) => {
	...
	//   const initialValue = localStorage.getItem('toggleValue') || '0';
	//   const [toggleValue, setToggleValue] = useState(initialValue);

	//   useEffect(() => {
	//     // Save the current toggle value to localStorage whenever it changes
	//     localStorage.setItem('toggleValue', toggleValue);
	//   }, [toggleValue]);

	//   const handleToggle = () => {
	//     // Toggle the value when the checkbox is clicked
	//     setToggleValue((prevValue) => (prevValue === '0' ? '1' : '0'));
	//   };
	...
	const admin_template_edit = async (event) => {
		...
		const versionCodeBefore = parseFloat(adminPanelSettingsEdit[0]?.version_code);
		console.log('versionCodeBefore:', versionCodeBefore);

		// const versionCodeAfter = versionCodeBefore + 0.1;
		// console.log('versionCodeAfter:', versionCodeAfter);

		const versionCodeAfter = parseFloat((versionCodeBefore + 0.1).toFixed(1));
		const adminPageListSettings = {
			...
			version_code: versionCodeAfter,
			// version_code: parseFloat(adminPanelSettingsEdit[0]?.version_code) + 0.1,
	...
	const loginTemplate = (event) => {
		const newValue = event.target.value
		setSelectedLoginTemplate(newValue);
	};
					...
					<div className='card'>
						<div className=' bg-light body-content' >
							<div className='   border-primary shadow-sm border-0' >
								<div class="card-header card_header_bg  custom-card-header py-1  clearfix  bg-gradient-primary border-0 text-white">
									<h5 class="card-title font-weight-bold mb-0  float-left mt-1 card_header_color">Admin Panel Settings</h5>
									<div class="card-title font-weight-bold mb-0 float-right">
										<Link href="/Admin/admin_template/admin_template_all" class="btn btn-sm btn-info card_header_color">Back to Admin panel settings List</Link>
									</div>
								</div>
								<div className=' card-body bg_card_body '>
									...
									<form action="" onSubmit={admin_template_edit}>
										<div className=' mt-4 '>
								...
								<label class="custom-control-label font-weight-bold" for="customSwitch1">Single Menu</label>
													...
													<div className="custom-control custom-switch">
														...
														{
															adminPanelSettingsEdit[0] && adminPanelSettingsEdit[0]?.left_menu === 1 &&
															<>
														...
														{
															adminPanelSettingsEdit[0] && adminPanelSettingsEdit[0]?.left_menu === 0 &&
															<>
														...
														{/* defaultChecked */}
														{/* // ={adminPanelSettingsEdit[0]?.left_menu === 1} */}
														{/* <input
											type="checkbox"
											className="custom-control-input"
											name="left_menu"
											id="customSwitch1"
											defaultChecked={adminPanelSettingsEdit[0]?.left_menu === 1}
											value={status ? 1 : 0}
										...
										<label className="custom-control-label font-weight-bold" htmlFor="customSwitch1">
											...
											<div class="form-group row px-4">
												...
												<div class="col-md-6">
													...
													{adminTemplateName && <p class="text-danger mb-0">{adminTemplateName}</p>
													}
													{sameName && <p class="text-danger mb-0">{sameName}</p>
													}
												</div>
											</div>
											<div class="form-group row px-4">
												...
												<div class="col-md-4">
													<div class="input-group input-group-sm mb-3">
														...
														<select required="" name="login_template" class="form-control form-control-sm custom-select trim integer_no_zero login_template" id="login_template" placeholder="Enter Expense Category"
															...
															defaultValue={adminPanelSettingsEdit[0]?.login_template_name ? adminPanelSettingsEdit[0]?.login_template_name : selectedLoginTemplate}
...
<div>
  {selectedFile.map((preview, index) => (
	...
	<button
												...
												<div className="col-md-6" >
													<div class="card-header  custom-card-header py-1  clearfix   border-0 text-white"
											...
											<div class="form-group row px-4">
												...
												<div class="col-md-7">
													<select required="" name="status" class="form-control form-control-sm trim integer_no_zero" id="status" placeholder="Enter Status"
														defaultValue={adminPanelSettingsEdit[0]?.status}
													>
...
//       />
//       <label className="custom-control-label font-weight-bold" htmlFor="customSwitch1">
//         {toggleValue === '0' ? 'Checked' : 'Unchecked'}
//       </label>
//     </div>
//   );
// };
