...
const TypeList = ({ searchParams }) => {
    ...
    console.log(moduleInfo.filter(moduleI => moduleI.controller_name === 'type'))
    const brandList = moduleInfo.filter(moduleI => moduleI.controller_name === 'type')
    ...
    const Category = module_settings.filter(moduleI => moduleI.table_name === 'type')
    ...
    const columnNames = types && types.length > 0 ? Object.keys(types[0]) : [];
    ...
    const type_excel_download = async () => {
        ...
        if (proceed) {
            axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/type/type_delete/${id}`)
                .then(response => {
                    ...
                    // Update the searchResults array by removing the deleted type
                    setSearchResults(prevResults => prevResults.filter(type => type.id !== id));
                })
                .catch(error => {
    ...
    const type_PDF_download = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Error generating PDF In type');
            }
                                    ...
                                    <form class="">
                                        <div class="col-md-10 offset-md-1">
                                            ...
                                            <div className='form-group row student'>
                                                <label class="col-form-label col-md-3"><strong>Type Name:</strong></label>
                                                <div class="col-md-3">
                                                    <input
                                                        value={searchQuery}
                                                        onChange={(e) => setSearchQuery(e.target.value)}
                                                        type="text" name="student_id" class="form-control form-control-sm  alpha_space student_id" id="student_id" placeholder="Type Name" />
                                                </div>
...
<input type="text" name="extra_column" class="form-control form-control-sm  alpha_space extra_column" id="extra_column" placeholder="Extra Column" />
                                        ...
                                        <div class="form-group row">
                                            <div class="offset-md-2 col-md-12 float-left">
                                                <input type="button" onClick={type_search} name="search" class="btn btn-sm btn-info search_btn mr-2" value="Search" />
                                                <input type="button" onClick={type_print} name="search" class="btn btn-sm btn-success print_btn mr-2" value="Print" />
                                                <button class="btn btn-sm btn-indigo pdf_btn mr-2" style={buttonStyles} onClick={type_PDF_download} disabled={loading}>
                    ...
                    <div className='card mb-4'>
                        <div class=" border-primary  shadow-sm border-0">
                            <div class="card-header   custom-card-header py-1  clearfix bg-gradient-primary text-white">
                                <h5 class="card-title card-header-color font-weight-bold mb-0  float-left mt-1">Type List</h5>
                                <div class="card-title card-header-color font-weight-bold mb-0  float-right"> <a href="/Admin/type/type_create?page_group=inventory" class="btn btn-sm btn-info">Create Type</a> </div>
                            </div>
                            {error && <div>Error: {error}</div>}
                            <div class="card-body" >
                                {loading ? <div className='text-center'>
                                    ...
                                    <div class="table-responsive">
                                        <table className="table table-bordered table-hover table-striped table-sm">
                                            ...
                                            <tbody>
                                                {searchResults.map((type, i) => (
                                                    <tr key={i}>
                                                        {selectedColumns.map((column, j) => (
                                                            <td key={j} style={['type_name', 'description'].includes(column) ? longTextStyle : {}}>
                                                                {column === 'serial' ? (
                                                                    // Rendering serial number if the column is 'serial'
                                                                    i + 1
                                                                ) : column === 'file_path' ? (
                                                                    ...
                                                                    <>
                                                                        <img
                                                                            ...
                                                                            style={{ width: '100px' }}
                                                                            src={`${process.env.NEXT_PUBLIC_API_URL}:5003/${type.file_path}`}
                                                                            alt="No Image"
                                                                        />
                                                                    </>
                                                                )
                                                                    ...
                                                                    column === 'status_id' ? (
                                                                        // Special handling for the 'status' column
                                                                        <>
                                                                            {type.status_id === 1 && <p>Active</p>}
                                                                            {type.status_id === 2 && <p>Inactive</p>}
                                                                            {type.status_id === 3 && <p>Pending</p>}
                                                                        </>
                                                                    )
                                                                        ...
                                                                        : column === 'action' ? (
                                                                            ...
                                                                            <div className="flex items-center ">
                                                                                <Link href={`/Admin/type/type_edit/${type.id}?page_group=${page_group}`}>
                                                                                ...
                                                                                <Link href={`/Admin/type/type_copy/${type.id}?page_group=${page_group}`}>
                                                                                ...
                                                                                {filteredBtnIconDelete.map((filteredBtnIconDelete => (
                                                                                    <button
                                                                                        ...
                                                                                        title='Delete'
                                                                                        onClick={() => type_delete_searching(type.id)}
                                                                                        style={{ width: "35px ", height: '30px', marginLeft: '5px', marginTop: '5px' }}
                                    ...
                                    <>
                                            ...
                                            <table className="table table-bordered">
                                                ...
                                                <tbody>
                                                    {pageUsers.length === 0 ? (
                                                        ...
                                                        pageUsers?.map((type, index) => (
                                                            <tr key={type.id}>
                                                                {selectedColumns?.map(column => (
                                                                    <td key={column} style={['type_name', 'description'].includes(column) ? longTextStyle : {}}>
                                                                        {column === 'serial' ?
                                                                            ...
                                                                            column === 'file_path' ? (
                                                                                ...
                                                                                column === 'status_id' ? (
                                                                                    type[column] === 1 ? 'Active' : type[column] === 2 ? 'Inactive' : type[column] === 3 ? 'Pending' : 'Unknown'
                                                                                )
                                                                                    :
                                                                                    column === 'action' ? (
                                                                                        <div className="flex items-center ">
                                        ...
                                        <div className=" d-flex justify-content-between">
                                            ...
                                            <div class="pagination float-right pagination-sm border">
                                                {
                                                    currentPage - 3 >= 1 && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/type/type_all?page=${1}`}>‹ First</Link>
                                                    )
                                                }
                                                {
                                                    currentPage > 1 && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/type/type_all?page=${activePage - 1}`}>&lt;</Link>
                                                    )
                                                }
                                                {
                                                    pageNumber.map((page) =>
                                                        <Link
                                                            key={page}
                                                            href={`/Admin/type/type_all?page=${page}`}
                                                            className={` ${page === activePage ? "font-bold bg-primary px-2 border-left py-1 text-white" : "text-primary px-2 border-left py-1"}`}
                                                        > {page}
                                                        </Link>
                                                    )
                                                ...
                                                {
                                                    currentPage + 3 <= totalPages && (
                                                        <Link className=" text-primary px-2 border-left py-1" href={`/Admin/type/type_all?page=${totalPages}`}>Last ›</Link>
                                                    )
                                                }
                                            </div>
