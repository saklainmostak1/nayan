...
//         supplier_id: '', item_name: '', quantity: '', unit_id: '', purchase_price: '', sale_price: '', total_amount: '', discount: '', due: '', paid_amount: '', purchase_invoice: '', purchase_date: '', invoice_id: '', purchase_type: '', created_by: created
//     });

//     // const productPrice = products.find(product => product.id == assetInfo.item_name)
...
//                                 <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Create Non Stock Purchase</h5>
...
// export default CreateNotStockPurchases;
...
const CreateNotStockPurchases = () => {
    ...
    // const prev_due = supplierLastDue?.payable_amount - supplierLastDue?.paid_amount - supplierLastDue?.due;
    ...
    const handleAddMore = () => {
        ...
        if (!isNaN(numToAddInt) && numToAddInt > 0) {
            ...
            for (let i = 0; i < numToAddInt; i++) {
                newInputValues.push({
                    item_name: '', quantity: '', unit_id: '', purchase_price: '', sale_price: '', created_by: created
                });
            }
    ...
    const totalSalePrice = fields.reduce((sum, field) => sum + (parseFloat(field.sale_price) || 0), 0);
    ...
    useEffect(() => {
        ...
        setAssetInfo(prevState => ({
            ...prevState,
            due: parseFloat(assetInfo.payable_amount) - parseFloat(assetInfo.discount) - parseFloat(assetInfo.paid_amount)
            // - parseFloat(assetInfo.discount ? assetInfo.discount : 0)
        }));
    ...
    const handleChange = (index, event) => {
        ...
        const product_id = newFields[index]['item_name'];
        if (product_id) {
                    ...
                    <div className='card'>
                        ...
                        <div className="card-default">
                            ...
                            <div className="card-header custom-card-header py-1  clearfix bg-gradient-primary text-white">
                                <h5 className="card-title card-header-color font-weight-bold mb-0  float-left mt-1">Create Non Stock Purchase</h5>
