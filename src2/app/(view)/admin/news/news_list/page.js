...
const NewsList = ({ searchParams }) => {
  ...
  const brandList = moduleInfo.filter(
    (moduleI) => moduleI.controller_name === "news"
  );
  ...
  const noticeCategoryAll_delete = (id) => {
    ...
    if (proceed) {
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/news/news_delete/${id}`,
        {
          method: "POST",
        }
      )
  ...
  const caregory_list = async () => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/news/news_list_paigination/${currentPage}/${dataPerPage}`;
  ...
  const columnNames = newsAll && newsAll.length > 0 ? Object.keys(newsAll[0]) : [];
  ...
  const news_search = () => {
    // setLoading(true);
    axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/news/news_search`, {
  ...
  const news_excel_download = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/news/news_search`, {
        fromDate, toDate, searchQuery, itemName
      });
  ...
  const news_pdf = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/news/news_pdf`, {
        method: 'POST',
        headers: {
  ...
  const news_Print_download = async () => {
    try {
      ...
      const html = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/news/news_print`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                      ...
                      <div class="form-group row student">
                        ...
                        <div className="col-md-3">
                          ...
                          <select required="" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} name="supplier_id" className="form-control form-control-sm mb-2" id="supplier_id">
                            ...
                            {
                              noticeCategoryAll.map((supplier) => (
                                <>
                                    ...
                                    {column === 'serial' ? (
                                      ...
                                      column === 'action' ? (
                                        ...
                                        <div className="flex items-center ">
                                          <Link
                                            href={`/Admin/news/news_edit/${noticeCategoryAll.id}?page_group=${page_group}`}
                                          >
                      ...
                      <>
                        ...
                        <div className="table-responsive ">
                            ...
                            <div class="pagination float-right pagination-sm border">
                              {currentPage - 3 >= 1 && (
                                <Link
                                  ...
                                  href={`/Admin/news/news_all?page=${1}`}
                                >
                                  ‹ First
                                </Link>
                              )}
                              {currentPage > 1 && (
                                <Link
                                  className=" text-primary px-2 border-left py-1"
                                  href={`/Admin/news/news_all?page=${activePage - 1}`}
                                >
                                  &lt;
                                </Link>
                              )}
                              {pageNumber.map((page) => (
                                <Link
                                  key={page}
                                  href={`/Admin/news/news_all?page=${page}`}
                              ...
                              {currentPage + 3 <= totalPages && (
                                <Link
                                  className=" text-primary px-2 border-left py-1"
                                  href={`/Admin/news/news_all?page=${totalPages}`}
                                >
                                  Last ›
                                </Link>
                              )}
                          ...
                          <table className="table  table-bordered table-hover table-striped table-sm mt-2">
                            <thead>
                              <tr>
                                <th style={longTextStyle}>Title</th>
                                <th>NewsCategory</th>
                              ...
                              {isLoading ? (
                                ...
                                pageUsers.map((noticeCategoryAll, i) => (
                                  <tr key={noticeCategoryAll.id}>
                                    ...
                                    <td>{noticeCategoryAll?.news_category}</td>
