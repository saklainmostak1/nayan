...
const PurchaseProductUpdate = ({ id }) => {
    ...
    const { data: purchaseProductSingle = [],
        ...
        queryFn: async () => {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/purchase_product/purchase_product_all/${id}`)
    ...
    useEffect(() => {
        ...
        setAssetInfo({
            product_id: purchaseProductSingle[0]?.product_id || '',
            quantity: purchaseProductSingle[0]?.quantity || '',
            unit_id: purchaseProductSingle[0]?.unit_id || '',
            purchase_price: purchaseProductSingle[0]?.purchase_price || '',
            sale_price: purchaseProductSingle[0]?.sale_price || '',
            modified_by: created
        });
    ...
    const brand_update = (e) => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/purchase_product/purchase_product_edit/${id}`, {
            method: 'POST',
            headers: {
...
export default PurchaseProductUpdate;
