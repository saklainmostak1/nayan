...
const ProductStockPrice = () => {
    ...
    const stock_pdf_download = async () => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/stock/stock_product_price_pdf`, {
                ...
                body: JSON.stringify({
                    searchResults: updatedSearchResults, orientation, fontSize, selectedPrintSize, totalAmountSum, totalQuantitySum
    ...
    const stock_print = async () => {
        try {
            ...
            const html = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/stock/stock_product_price_print`, {
                ...
                body: JSON.stringify({
                    searchResults, selectedPrintSize, orientation, fontSize, extraColumnValue, totalAmountSum, totalQuantitySum
                }),
            });
    ...
    const stock_excel_download = async () => {
        try {
            ...
            // Filter the data
            const filteredColumns = updatedSearchResults.map((category, index) => ({
                'SL No.': index + 1,
                'Product Name': category.product_name,
                'Unit Price': category.sale_price,
                'Quantity': category.purchase_product_quantity - category.sale_product_quantity,
                'Total Price': category.sale_price * (category.purchase_product_quantity - category.sale_product_quantity),
            }));
    ...
    const stock_word_download = async () => {
        try {
            ...
            const dataRows = await Promise.all(searchResults.map(async (category, index) => {
                ...
                const totalPrice = category.sale_price * (category.purchase_product_quantity - category.sale_product_quantity)
                ...
                return new TableRow({
                    children: [
                        ...
                        new TableCell({
                            children: [new Paragraph({ text: `${totalPrice}` })], // Contact Number
                            borders: {},
                        }),
            ...
            const footerRow = new TableRow({
                children: [
                    new TableCell({
                        children: [new Paragraph({ text: '' })], // Empty for SL No.
                        borders: {},
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: 'Total', bold: true })], // Label for totals
                        borders: {},
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: '' })], // Empty for Unit Price
                        borders: {},
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: `${totalQuantitySum}`, bold: true })], // Total Quantity
                        borders: {},
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: `${totalAmountSum}`, bold: true })], // Total Amount
                        borders: {},
                    }),
                ],
            });
            ...
            const table = new Table({
                rows: [headerRow, ...dataRows, footerRow], // Include footerRow at the end
                width: {
                    size: 100,
                ...
                <div className='col-12 p-4'>
                        ...
                        <div className="body-content bg-light">
                            <div className="border-primary shadow-sm border-0">
                                <div className="card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                                    <h5 className="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Stock Product Price Search</h5>
                                </div>
                    ...
                    <div className='card'>
                        <div className="body-content bg-light">
                            <div className="border-primary shadow-sm border-0">
                                <div className="card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                                    <h5 className="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Stock Product Price</h5>
                                                                ...
                                                                {isLoading ? <div className='text-center'>
                                                                    ...
                                                                    searchResults.map((loan, i) => (
                                                                        <tr key={loan.id}>
                                                                            ...
                                                                            <td>    {loan.sale_price * (loan.purchase_product_quantity - loan.sale_product_quantity)}</td>
                                        ...
                                        <div class="card-body">
                                            <div className='table-responsive'>
                                                ...
                                                <table className="table  table-bordered table-hover table-striped table-sm">
                                                    ...
                                                    <tr>
                                                        <td colSpan="3"></td>
                                                        <th>{totalQuantitySumList}</th>
                                                        <th>{totalAmountSumList}</th>
                                                    </tr>
                                                </table>
