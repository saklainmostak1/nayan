...
const ModelList = ({searchParams}) => {
    ...
    console.log(moduleInfo.filter(moduleI => moduleI.controller_name === 'model'))
    const brandList = moduleInfo.filter(moduleI => moduleI.controller_name === 'model')
    ...
    const Category = module_settings.filter(moduleI => moduleI.table_name === 'model')
    ...
    const columnNames = models && models.length > 0 ? Object.keys(models[0]) : [];
 ...
 const model_search = () => {
     setLoading(true);
     axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/model/model_search`, {
         multiSearch,
         selectedColumns,
         searchQuery,
         statusFilter,
...
const all_models = async () => {
 const url = `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/model/model_all/${currentPage}/${dataPerPage}`;
...
const activePage = searchParams?.page ? parseInt(searchParams.page) : 1;
    ...
    const model_print = async () => {
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/model/model_search`, {
                selectedColumns,
                searchQuery,
                statusFilter,
                selectedOrder,
...
const model_excel_download = async() => {
...
const model_word_download = async () => {
        ...
        if (proceed) {
            axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/model/model_delete/${id}`)
                .then(response => {
                    ...
                    // Update the searchResults array by removing the deleted model
                    setSearchResults(prevResults => prevResults.filter(model => model.id !== id));
                })
                .catch(error => {
    ...
    const model_PDF_download = async () => {
        setLoading(true);
        const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/model/model_search`, {
            selectedColumns,
            searchQuery,
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/model/model_pdf`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                throw new Error('Error generating PDF In model');
            }
            ...
            <div class=" body-content bg-light ">
                ...
                <div class=" border-primary shadow-sm border-0">
                    <div class=" card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                        <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Model Search</h5>
                        <div class="card-title font-weight-bold mb-0 card-header-color float-right">

                            <Link href={`/Admin/model/model_create?page_group=${page_group}`} class="btn btn-sm btn-info">Back To Create Model</Link>
                        </div>
                    </div>
...
<Select
                        ...
                        <label class="col-form-label col-md-3"><strong>Model Name:</strong></label>
                            <div class="col-md-3">
                                <input
                                    value={searchQuery} 
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    type="text" name="student_id" class="form-control form-control-sm  alpha_space student_id" id="student_id" placeholder="Model Name" />
                            </div>
...
<label class="col-form-label col-md-3"><strong>To Date:</strong></label>
<div class="col-md-3">
    <DatePicker
        selected={toDate}
        dateFormat="dd-MM-yyyy" // Set the date format
...
<div class="col-md-4">
    <input type="text" name="extra_column" class="form-control form-control-sm  alpha_space extra_column" id="extra_column" placeholder="Extra Column" />
</div> */}
            ...
            <option value="100%" selected="">100% Zoom</option>
                    ...
                    <div class="form-group row">
                        <div class="offset-md-2 col-md-12 float-left">
                            <input type="button" onClick={model_search} name="search" class="btn btn-sm btn-info search_btn mr-2" value="Search" />
                            <input type="button" onClick={model_print} name="search" class="btn btn-sm btn-success print_btn mr-2" value="Print" />
                            <button class="btn btn-sm btn-indigo pdf_btn mr-2" style={buttonStyles} onClick={model_PDF_download} disabled={loading}>
...
/>
            ...
            <div class=" border-primary  shadow-sm border-0">
                <div class="card-header   custom-card-header py-1  clearfix bg-gradient-primary text-white">
                    <h5 class="card-title card-header-color font-weight-bold mb-0  float-left mt-1">Model List</h5>
                    <div class="card-title card-header-color font-weight-bold mb-0  float-right">
                        <Link href={`/Admin/model/model_create?page_group=${page_group}`} className="btn btn-sm btn-info">Back to Model Create</Link>
                    </div>
                </div>
          ...
          <FontAwesomeIcon style={{
                                ...
                                { searchResults.map((brand, i) => (
                                    <tr key={i}>
                                        {selectedColumns.map((column, j) => (
                                            <td key={j} style={['model_name', 'description'].includes(column) ? longTextStyle : {}}>
                                                {column === 'serial' ? (
                                                    // Rendering serial number if the column is 'serial'
                                                    i + 1
                                                ) : column === 'file_path' ? (
                                                    ...
                                                    column === 'status_id' ? (
                                                        ...
                                                        : column === 'action' ? (
                                                            ...
                                                            <div className="flex items-center ">
                                                                <Link href={`/Admin/model/model_edit/${brand.id}?page_group=${page_group}`}>
                                                                ...
                                                                <Link href={`/Admin/model/model_copy/${brand.id}?page_group=${page_group}`}>
                                                                ...
                                                                {filteredBtnIconDelete.map((filteredBtnIconDelete => (
                                                                    <button
                                                                        ...
                                                                        title='Delete'
                                                                        onClick={() => model_delete_searching(brand.id)}
                                                                        style={{ width: "35px ", height: '30px', marginLeft: '5px', marginTop: '5px' }}
                 ...
                 <table className="table table-bordered">
                                        ...
                                        <tbody>
                                            {pageUsers.length === 0 ? (
                                                ...
                                                pageUsers?.map((model, index) => (
                                                    <tr key={model.id}>
                                                        {selectedColumns?.map(column => (
                                                            <td key={column} style={['model_name', 'description'].includes(column) ? longTextStyle : {}}>
                                                                {column === 'serial' ? (
                                                                    ...
                                                                    column === 'file_path' ? (
                                                                        ...
                                                                        <>
                                                                            <img
                                                                                className=" img-thumbnail"
                                                                                style={{ width: '100px' }}
                                                                                src={`${process.env.NEXT_PUBLIC_API_URL}:5003/${model.file_path}`}
                                                                                alt="No Image"
                                                                            />
                                                                        </>
                                                                    ) :
                                                                    column === 'status_id' ? (
                                                                        model[column] === 1 ? 'Active' : model[column] === 2 ? 'Inactive' : model[column] === 3 ? 'Pending' : 'Unknown'
                                                                    )
                                                                    :
                                                                        column === 'action' ? (
                                                                            <div className="flex items-center ">
                                                                                <Link href={`/Admin/model/model_edit/${model.id}?page_group=${page_group}`}>
                                                                                    {filteredBtnIconEdit?.map((filteredBtnIconEdit => (
                                                                                        <button
                                                                                            key={filteredBtnIconEdit.id}
                                                                                            title='Edit'
                                                                                ...
                                                                                <Link href={`/Admin/model/model_copy/${model.id}?page_group=${page_group}`}>
                                                                                    {filteredBtnIconCopy.map((filteredBtnIconEdit => (
                                                                                        <button
                                                                                            key={filteredBtnIconEdit.id}
                                                                                            title='Copy'
            ...
            <div className=" d-flex justify-content-between">
                ...
                <div class="pagination float-right pagination-sm border">
                    {
                        currentPage - 3 >= 1 && (
                            <Link className=" text-primary px-2 border-left py-1" href={`/Admin/model/model_all?page=${1}`}>‹ First</Link>
                        )
                    }
                    {
                        currentPage > 1 && (
                            <Link className=" text-primary px-2 border-left py-1" href={`/Admin/model/model_all?page=${activePage - 1}`}>&lt;</Link>
                        )
                    }
                    {
                        pageNumber.map((page) =>
                            <Link
                                key={page}
                                href={`/Admin/model/model_all?page=${page}`}
                                className={` ${page === activePage ? "font-bold bg-primary px-2 border-left py-1 text-white" : "text-primary px-2 border-left py-1"}`}
                            > {page}
                            </Link>
                        )
                    ...
                    {
                        currentPage + 3 <= totalPages && (
                            <Link className=" text-primary px-2 border-left py-1" href={`/Admin/model/model_all?page=${totalPages}`}>Last ›</Link>
                        )
                    }
                </div>
...
//             try {
//                 const response = await fetch('http://192.168.0.119:5002/Admin/model/model_search', {
//                     method: 'POST',
//                     headers: {
//                         'Content-Type': 'application/json'
//                     },
...
//                 // Handle results, e.g., setFilteredCategories(data.results);
//             } catch (error) {
...
//     return (
//         <div>
//             <label htmlFor="selectedColumns">Select Columns:</label>
//             <select id="selectedColumns" multiple onChange={(e) => setSelectedColumns(Array.from(e.target.selectedOptions, option => option.value))}>
...
//             <input type="text" id="statusFilter" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} />
//             <br />

//             <label htmlFor="selectedOrder">Select Order:</label>
//             <select id="selectedOrder" value={selectedOrder} onChange={(e) => setSelectedOrder(e.target.value)}>
//                 {/* Options for selecting order */}
//             </select>
//             <br />

//             <button onClick={model_search}>Search</button>

//             <div>
//                 {
//                     data.map(datas =>
//                         <>
//                         <p>{datas.brand_name}</p>
//                         </>
