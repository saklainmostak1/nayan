...
//         const _path = 'loan_payment/' + time + '/' + newName;
...
//         axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5003/loan_payment/loan_payment_image`, formData)
//             .then(res => {
//                 console.log(res);
//             })
...
//                                 <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Update Loan Payment</h5>
//                                 <div class="card-title font-weight-bold mb-0 card-header-color float-right">
//                                     <Link href={`/Admin/loan_payment/loan_payment_all?page_group=${page_group}`} class="btn btn-sm btn-info">Back Loan Payment List</Link></div>
//                             </div>
...
// export default LoanPaymentUpdate;
...
const LoanPaymentUpdate = ({ id }) => {
    ...
    const [assetInfo, setAssetInfo] = useState({
        loan: '', account: '', reference: '', amount: '', interest: '', aviable_balance: '', due: '', payable_amount: '', payment_date: '', note: '', status: '', modified_by: created, img: '',remaining_balance:''
    });
    ...
    // const result = loanAll.reduce((acc, curr) => {
    //     // If the account already exists, add the payable_amount to the existing total
    //     if (acc[curr.account]) {
    //       acc[curr.account] += curr.payable_amount;
    //     } else {
    //       // If the account doesn't exist, set it with the current payable_amount
    //       acc[curr.account] = curr.payable_amount;
    //     }
    //     return acc;
    //   }, {});
    ...
    const totalPayableForAccount6 = loanAll
        .filter(item => item.account == assetInfo.account && item.id == assetInfo.loan)  // Filter the data for account '6'
    ...
    const totalPayableForLoanPaymentList = loanPaymentList
        .filter(item => item.account == assetInfo.account && item.loan_id == assetInfo.loan)  // Filter the data for account '6'
        .reduce((sum, curr) => sum + curr.payable_amount, 0);  // Sum up payable_amount
    ...
    const loan_payment_create = (e) => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/loan_payment/loan_payment_update/${id}`, {
            ...
            .then(data => {
                ...
                if (data) {
                    sessionStorage.setItem("message", "Data Update successfully!");
                    router.push(`/Admin/loan_payment/loan_payment_all?page_group=${page_group}`);
                }
            })
            .catch(error => {
                console.error('Error updating brand:', error);
                                                    ...
                                                    <select
                                                        ...
                                                        <option value=''>Select Loan</option>
                                                        {
                                                            loanAll.map(sta =>
                                                                ...
                                                                <option value={sta.id}>{sta.loan_authority_name}-{sta.payable_amount}-{`(Ref: ${sta.reference})`}</option>
                                                            </>
                                                            )
                                                        }
                                                    </select>
