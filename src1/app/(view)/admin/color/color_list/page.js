...
const ColorList = ({searchParams}) => {
    ...
    console.log(moduleInfo.filter(moduleI => moduleI.controller_name === 'color'))
    const brandList = moduleInfo.filter(moduleI => moduleI.controller_name === 'color')
    ...
    const Category = module_settings.filter(moduleI => moduleI.table_name === 'color')
...
const color_excel_download = async() => {
    ...
    if (proceed) {
        axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/color/color_delete/${id}`)
            .then(response => {
                ...
                // Update the searchResults array by removing the deleted color
                setSearchResults(prevResults => prevResults.filter(color => color.id !== id));
            })
            .catch(error => {
...
const [errorr, setErrorr] = useState(null);
    const color_PDF_download = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Error generating PDF In color');
            }
            ...
            <div class=" body-content bg-light">
                ...
                <div class=" border-primary shadow-sm border-0">
                    <div class=" card-header py-1 custom-card-header clearfix bg-gradient-primary text-white">
                        <h5 class="card-title font-weight-bold mb-0 card-header-color float-left mt-1">Color Search</h5>
...
<Select
                        ...
                        <label class="col-form-label col-md-3"><strong>Color Name:</strong></label>
                            <div class="col-md-3">
                                <input
                                    value={searchQuery} 
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    type="text" name="student_id" class="form-control form-control-sm  alpha_space student_id" id="student_id" placeholder="Color Name" />
                            </div>
            ...
            <option value="100%" selected="">100% Zoom</option>
                    ...
                    <div class="form-group row">
                        <div class="offset-md-2 col-md-12 float-left">
                            <input type="button" onClick={color_search} name="search" class="btn btn-sm btn-info search_btn mr-2" value="Search" />
                            <input type="button" onClick={color_print} name="search" class="btn btn-sm btn-success print_btn mr-2" value="Print" />
                            <input style={buttonStyles} type="button" name="search" class="btn btn-sm btn-indigo pdf_btn mr-2" value="Generate PDF" onClick={color_PDF_download} />
{errorr && <p>Error: {errorr}</p>}
                          
                            <input
    type="button"
    onClick={color_excel_download}
    name="search"
...
/>
            ...
            <div class=" border-primary  shadow-sm border-0">
                <div class="card-header   custom-card-header py-1  clearfix bg-gradient-primary text-white">
                    <h5 class="card-title card-header-color font-weight-bold mb-0  float-left mt-1">Color List</h5>
                    <div class="card-title card-header-color font-weight-bold mb-0  float-right"> <Link href={`/Admin/color/color_create?page_group=${page_group}`} class="btn btn-sm btn-info">Back To Create Color</Link> </div>
                </div>
      ...
      searchResults.length > 0 ? (
                    <div  class="table-responsive">
                        ...
                        <table id='mytable'  className="table table-bordered table-hover table-striped table-sm">
                            ...
                            <tbody>
                                {   searchResults.map((color, i) => (
                                    <tr key={i}>
                                        {selectedColumns.map((column, j) => (
                                            <td key={j} style={['color_name', 'description'].includes(column) ? longTextStyle : {}}>
                                                {column === 'serial' ? (
                                                    // Rendering serial number if the column is 'serial'
                                                    i + 1
                                                ) : column === 'file_path' ? (
                                                    ...
                                                    <>
                                                        <img
                                                            ...
                                                            style={{ width: '100px' }}
                                                            src={`${process.env.NEXT_PUBLIC_API_URL}:5003/${color.file_path}`}
                                                            alt="No Image"
                                                        />
                                                    </>
                                                )
                                                    ...
                                                    column === 'status_id' ? (
                                                        // Special handling for the 'status' column
                                                        <>
                                                            {color.status_id === 1 && <p>Active</p>}
                                                            {color.status_id === 2 && <p>Inactive</p>}
                                                            {color.status_id === 3 && <p>Pending</p>}
                                                        </>
                                                    )
                                                        ...
                                                        : column === 'action' ? (
                                                            ...
                                                            <div className="flex items-center ">
                                                                <Link href={`/Admin/color/color_edit/${color.id}?page_group=${page_group}`}>
                                                                ...
                                                                <Link href={`/Admin/color/color_copy/${color.id}?page_group=${page_group}`}>
                                                                ...
                                                                {filteredBtnIconDelete.map((filteredBtnIconDelete => (
                                                                    <button
                                                                        ...
                                                                        title='Delete'
                                                                        onClick={() => color_delete_searching(color.id)}
                                                                        style={{ width: "35px ", height: '30px', marginLeft: '5px', marginTop: '5px' }}
                ...
                <table className="table table-bordered">
                                        ...
                                        <tbody>
                                            {pageUsers.length === 0 ? (
                                                ...
                                                pageUsers?.map((color, index) => (
                                                    <tr key={color.id}>
                                                        {selectedColumns?.map(column => (
                                                            <td key={column} style={['color_name', 'description'].includes(column) ? longTextStyle : {}}>
                                                                {column === 'serial' ? (
                                                                    ...
                                                                    column === 'status_id' ? (
                                                                        color[column] === 1 ? 'Active' : color[column] === 2 ? 'Inactive' : color[column] === 3 ? 'Pending' : 'Unknown'
                                                                    )
                                                                    :
                                                                        column === 'action' ? (
                                                                            <div className="flex items-center ">
            ...
            <div className=" d-flex justify-content-between">
                ...
                <div class="pagination float-right pagination-sm border">
                    {
                        currentPage - 3 >= 1 && (
                            <Link className=" text-primary px-2 border-left py-1" href={`/Admin/color/color_all?page=${1}`}>‹ First</Link>
                        )
                    }
                    {
                        currentPage > 1 && (
                            <Link className=" text-primary px-2 border-left py-1" href={`/Admin/color/color_all?page=${activePage - 1}`}>&lt;</Link>
                        )
                    }
                    {
                        pageNumber.map((page) =>
                            <Link
                                key={page}
                                href={`/Admin/color/color_all?page=${page}`}
                                className={` ${page === activePage ? "font-bold bg-primary px-2 border-left py-1 text-white" : "text-primary px-2 border-left py-1"}`}
                            > {page}
                            </Link>
                        )
                    ...
                    {
                        currentPage + 3 <= totalPages && (
                            <Link className=" text-primary px-2 border-left py-1" href={`/Admin/color/color_all?page=${totalPages}`}>Last ›</Link>
                        )
                    }
                </div>
...
//             const filteredCategories = colors.filter(
//                 (category) =>
//                     String(category.color_name).toLowerCase().includes(query) &&
...
//         const doc = new jsPDF();
//         doc.autoTable({
//             head: [['Serial', ...selectedColumns.map(column => formatString(column))]],
//             body: filteredCategorys.map((category, index) => [index + 1, ...selectedColumns.map(column => category[column])])
//         });
...
//         const bodyData = await Promise.all(filteredCategorys.map(async (category, i) => {
//             const rowData = await Promise.all(columnsToDisplay.map(async column => {
//                 if (column === 'serial') {
...
//                         return 'Image Not Found'; // Placeholder for error
//                     }
//                 } else if (column === 'status_id') {
//                     // Handle status ID
...
//         // editWindow.document.write('<html><head><title>Brand List</title></head><style> @page { size: ' + orientation + '; width: ' + pageWidth + '; height: ' + pageHeight + ' }</style><body>');

//         editWindow.document.write('<h1 style="text-align: center">Color List</h1>');
//    // Add CSS styles to center the heading
...
//             unit: 'mm',
//             format: [pageWidth, pageHeight] // Set page width and height based on selected print size
//         });
...
//                             <Link href={`/Admin/color/color_create?page_group=${page_group}`} class="btn btn-sm btn-info">Back To Create Category</Link>
//                         </div>
//                     </div>
...
//                                 <div class="form-group row student">
//                                     <label class="col-form-label col-md-2"><strong>Color Name:</strong></label>
//                                     <div class="col-md-4">
//                                         <input
//                                             value={searchQuery}
//                                             onChange={(e) => setSearchQuery(e.target.value)}
...
//                                     </div>
//                                     <label class="col-form-label col-md-2"><strong>Status:</strong></label>
//                                     <div className="col-md-4">
//                                         <select
//                                             name="statusFilter"
//                                             className="form-control form-control-sm integer_no_zero lshift"
...
//                                     <label class="col-form-label col-md-2"><strong>Extra Column:</strong></label>
//                                     <div class="col-md-4">
//                                         <input type="text" name="student_ids" class="form-control form-control-sm  alpha_space extra_column" id="extra_column" placeholder="Student ID" />
//                                     </div>
//                                 </div>
...
//                                     <input type="button" name="search" onClick={generatePDF} class="btn btn-sm btn-success print_btn mr-2" value="Print" />
//                                     <input type="button" onClick={downloadPdf} style={buttonStyles} name="search" class="btn btn-sm btn-indigo pdf_btn mr-2" value="Download PDF" />

//                                     <input type="button" name="search" class="btn btn-sm btn-secondary excel_btn mr-2" value="Download Excel " />
//                                 </div>
//                             </div>
//                         </form>
//                         {/* <div class="col-md-12 clearfix loading_div text-center" style={{ overflow: 'hidden', }}>
//                             <i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i>
//                         </div> */}
