...
const CompanyTypeList = ({ searchParams }) => {
  const {
  ...
  const brandList = moduleInfo.filter(
    (moduleI) => moduleI.controller_name === "company_type"
  );
  ...
  const caregory_list = async () => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/company_type/company_type_list_paigination/${currentPage}/${dataPerPage}`;
  ...
  const photogalleryCategoryAll_delete = (id) => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}:5002/Admin/company_type/company_type_delete/${id}`,
      {
        method: "POST",
      }
    )
                      ...
                      <div class="pagination float-right pagination-sm border">
                        {currentPage - 3 >= 1 && (
                          <Link
                            ...
                            href={`/Admin/company_type/company_type_all?page=${1}`}
                          >
                            ‹ First
                          </Link>
                        )}
                        ...
                        {currentPage + 3 <= totalPages && (
                          <Link
                            className=" text-primary px-2 border-left py-1"
                            href={`/Admin/company_type/company_type_all?page=${totalPages}`}
                          >
                            Last ›
                          </Link>
                        )}
                            ...
                            <tr key={item.id}>
                              ...
                              <td>
                                <div className="flex items-center">
                                  <Link
                                    href={`/Admin/company_type/company_type_edit/${item.id}?page_group=${page_group}`}
                                  >
